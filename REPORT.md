...
**Note on the Literature:** To the best of our knowledge, we have not found this explicit formulation in the classical and contemporary literature on the problem (such as in works by Terras, Lagarias, Eliahou, Simons–de Weger, and Tao). While the non-existence of a modular Lyapunov function over all the integers is an expected corollary of the existence of cycles among the negatives, the rigorous demonstration that the fixed point $-1 \in \mathbb{Z}_2$ constitutes, on its own, a *structural* and *irreducible* topological obstruction to potentials of period $2^j$ seeks to bring a distinct perspective. It aims to formally establish why approaches based strictly on finite residues face insurmountable barriers.
