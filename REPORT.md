...
* **Initial Positive Bounds:** Closure to 1 was attested for all non-zero naturals up to and including $200{,}000$ under ascending linear detection; no cyclic deviation was documented. (This is far below the published record $2^{68}$, Barina 2021; it is included solely as a self-contained, re-runnable hypothesis for the Diophantine exclusion below.)
...
* Under the R1-verified hypothesis ($N = 200{,}000$), the Diophantine certificate implies that any nontrivial positive cycle would have more than $428$ odd steps and more than $676$ total steps. Under Barina's published hypothesis ($N = 2^{68}$), the respective bounds become more than $8{,}517{,}411{,}709$ odd steps and more than $13{,}457{,}510{,}500$ total steps (and therefore elements). These are conditional consequences of the verification hypotheses, not additional proofs of convergence.
...
* **Absolute Measures ($\tau_k$ Wasserstein):** For $k \in \{2, 3, 4\}$, the exact fractional values returned were $\tau_2 = 5/21$, $\tau_3 = 455/1387$, and $\tau_4 = 7635497415/22906579627 \approx 0.33333206$. The three values satisfy $\tau_2 < \tau_3 < \tau_4 \leq 1/3$; this finite comparison is consistent with the global bound, but does not itself establish an asymptotic limit.
