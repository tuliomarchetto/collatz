...
**Note on Originality:** A review of the classical and contemporary literature on the problem (including Terras, Lagarias, Eliahou, Simons–de Weger, and Tao) does not reveal the presence of this result. While the non-existence of a modular Lyapunov function over all the integers is an expected corollary of the existence of cycles among the negatives, the rigorous demonstration that the fixed point $-1 \in \mathbb{Z}_2$ constitutes, on its own, a *structural* and *irreducible* topological obstruction to potentials of period $2^j$ is a new theorem. It formally establishes why approaches based strictly on finite residues fail on the conjecture.
...
Below we report the absolute quantifications obtained by applying the computational engines detailed in Part II.

### Global Computational Sieve and Inverse Tree
* **Initial Positive Bounds:** Closure to 1 was attested for all non-zero naturals below $200{,}000$ under ascending linear detection; no cyclic deviation was documented.
...
* **Empirical Rank Collapse ($3^k$):** Evaluating the Syracuse operator at $k=3$ (ring $\mathbb{Z}/27\mathbb{Z}$), spectral convergence precisely zeroed out all supplementary eigenvalues. The measure converged irrefutably to the rational proportions $1/3$ and $2/3$, breaking trivial symmetries and proving the collapses of Theorem 1 at a finite test dimension.
...
* **Practical Identification of $-1 \bmod 2^j$:** Running the *Karp Mean Cycle* analysis for iterated moduli in the range $j = 5 \dots 10$, the returned mean settled, in a computationally unconditional way, at the constant peak of $0.584962$ (the floating-point precision of $\log_2 3 - 1$). In $100\%$ of the runs performed, the subgraph responsible for the disruption contained one and only one residue-class signature: the one generated strictly by the transient modulus of the integer $-1$.
