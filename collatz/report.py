...
def run_all(verify_limit: int = 200_000, cycle_len: int = 14,
    ...
    if res.all_converge:
        add(f"- Todo n ≤ {verify_limit:,} converge a 1 (verificado agora; "
            f"literatura: ≈ 2^71). Nenhum contraexemplo.")
        n_rec, s_rec = res.stopping_records[-1]
        add(f"- Recorde de tempo de parada total: n = {n_rec} com {s_rec} passos "
            f"(crescimento observado ~ c·log n, c ≈ {s_rec / math.log(n_rec):.1f}).")
        n_exc, v_exc = res.excursion_records[-1]
        add(f"- Recorde de excursão: n = {n_exc} atinge {v_exc:,} "
            f"(razão pico/n² ≈ {v_exc / n_exc**2:.2f}; escala conjecturada ~ n²).")
    else:
        add(f"- ⚠ CONTRAEXEMPLO CANDIDATO: n = {res.counterexample}, ciclo = {res.cycle}")
    resm = search.verify_range(min(verify_limit, 50_000), d=-1)
    add(f"- Validação do detector no sistema irmão 3n−1: "
    ...
    for c in cy1:
        tag = "trivial" if 1 in c else ("NEGATIVO" if min(c) < 0 else "⚠ NÃO TRIVIAL POSITIVO")
        add(f"    - {_fmt_cycle(c)}  [{tag}]")
    add("- Nenhum ciclo positivo não trivial — consistente com a conjectura; "
    ...
    add(f"- Sistema irmão 3n+5 (positivos): {len(cy5)} ciclos, p.ex. "
    ...
    add(f"- Bijeção Q_k: Z/2^k ↔ vetores de paridade, k = {terras_k}: "
        f"{'confirmada' if ok_b else '⚠ FALHOU'}.")
    add(f"- Conjugação com o shift (paridade de T(n) = shift do vetor de n): "
        f"{'confirmada' if ok_s else '⚠ FALHOU'}.")
    add(f"- Censo de paridades = Binomial(k, 1/2) exata: "
    ...
    add(f"- Medida do conjunto 'ainda crescente': {m_half:.4f} após k = "
    ...
    add(f"- Fatoração da dinâmica: T(n) mod m é função de n mod m₁ com m₁ mínimo; "
    ...
    add(f"- Classes transientes mod 3: {sorted(tr3)}; mod 9: {sorted(tr9)} — "
    ...
    add(f"- Partição conservada mais grossa mod 8 (bissimulação a partir da "
    ...
    add(f"- Ciclo ótimo (resíduos com sinal): {verd['cycle_as_signed']} — é o ponto "
        "fixo 2-ádico −1 (ou os ciclos de −5/−17 em outros ótimos locais).")
    add("- ACHADO ESTRUTURAL: nenhuma função de Lyapunov da forma "
    ...
    add(f"- Auto-similaridade x ↦ 5x conjuga 3n+1 com 3n+5 nos múltiplos de 5: "
    ...
    add(f"- Automorfismos afins do grafo de transição mod 16: {autos} — grupo "
    ...
    add(f"- ACHADO: a uniforme NÃO é estacionária (mod 3^{spectral_k}: {uni}). "
    ...
    add(f"- Segundo autovalor |λ₂| ≈ {gap:.4f}: é exatamente ZERO — verificação "
        f"exata do colapso de posto: {'confirmada' if mem else 'FALHOU'} "
        "(linhas idênticas para r ≡ r' mod 3^(k-1) ⇒ P^k tem posto 1).")
    add("- ACHADO ESTRUTURAL: a cadeia perde TODA a memória mod 3^k em exatamente "
