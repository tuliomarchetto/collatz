...
The content of Theorem~\ref{thm:main} is the precise packaging of this phenomenon as a \emph{nonexistence} statement for the class of Definition~\ref{def:lyap}: the hypothesis is imposed \emph{only on $\Z_+$}, where no cycle other than $\{1,2\}$ is known, and the failure is nevertheless unconditional --- caused by the projection of the single $2$-adic point $-1$, not by any cycle in the domain of the hypothesis. We make no claim of priority; we regard the theorem as a formalization of a known heuristic, in the spirit of the average-descent results of \cite{terras1976,tao2022}. The closest reputable relative is a no-go of a different kind: Kohl \cite{kohl2008} shows the permutation conjugating an almost-contracting residue-class-wise affine map cannot itself be such a map --- a statement about a \emph{conjugacy}, not an additive $\log_2 n + w(n\bmod 2^j)$ potential.

\textbf{The question motivating this note:} is this formulation --- nonexistence of a strictly decreasing $V(n)=\log_2 n + w(n \bmod 2^j)$ \emph{on the positive odd integers}, with the robustness of Corollary~\ref{cor:robust} --- stated explicitly anywhere in the $3x+1$ literature? A systematic search (Lagarias's bibliographies, arXiv, citation chasing) did not locate it; we ask for a pointer to any source it missed. References or corrections are most welcome.
