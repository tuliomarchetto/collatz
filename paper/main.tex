...
\begin{figure}[h]
    ...
    \caption{\raggedright The residue graph $G_4$ on $\Z/16\Z$ (Section~\ref{sec:karp}), generated from the laboratory code by \texttt{paper/make\_residue\_graph\_figure.py} (via \texttt{collatz.invariants.transition\_graph\_mod2j}). Each residue~$r$ has the two successors $T(r)\bmod 16$ and $T(r+16)\bmod 16$. Solid arcs leave odd residues (shaded) and carry weight $\log_2 3-1\approx 0.585$; dotted arcs leave even residues and carry weight~$-1$. Exactly two self-loops: at~$0$ (weight~$-1$) and at $15\equiv -1\pmod{16}$ (dark, weight $\log_2 3-1>0$) --- the uncompensable positive-gain loop of Theorem~\ref{thm:main}, returned by Karp's algorithm as an optimal cycle at every tested level (Section~\ref{sec:results-karp}).}
...
If $n$ is even, $n=2m$ with $1\le m<2^{L-1}$, then $T^{L}(n)=T^{L-1}(m)$ with the same $a$. The inductive hypothesis yields $T^{L-1}(m)\le 2\max(1,3^{a}/2^{L-1})\,m$, so $T^{L}(n)/n\le \max(1,3^{a}/2^{L-1})$. The comparison
\[
\max\bigl(1,3^{a}/2^{L-1}\bigr) \;\le\; 2\max\bigl(1,3^{a}/2^{L}\bigr)
\]
is the same three-case check as in the even branch of the companion estimates to Lemma~\ref{lem:residual} (cf.\ the large-seed even reduction).
...
\;:=\;
\sup\Bigl\{\,
  \textstyle\sum_{i=0}^{r-1} \Gamma_\sigma(n_i)
  \;:\;
  n_0,\dots,n_r \text{ a $\sigma$-walk avoiding $E$}
\,\Bigr\} \\
\;\in\; [0, +\infty].
\end{multline*}
\end{definition}
...
\;:=\;
\sup\Bigl\{\,
  \textstyle\sum_{i=0}^{r-1}\Gamma_\sigma(m_i)
  \;:\;
  m_0 = n,\\
  \qquad m_0,\dots,m_r \text{ a $\sigma$-walk avoiding $E$}
\,\Bigr\}
\end{multline*}
...
The Python package \texttt{collatz}, the test suite, the reproduction script \texttt{reproduce\_paper\_results.py}, and this manuscript are available at \url{https://github.com/tuliomarchetto/collatz} under the MIT license (code) and CC~BY~4.0 (text). All results reproduce with CPython $\ge 3.9$ with no third-party dependencies. Figure~\ref{fig:residue_graph} is itself a generated artifact of the package, certified by the reproduction script. The exact repository state corresponding to this manuscript is archived on Zenodo (DOI: \href{https://doi.org/10.5281/zenodo.21423111}{10.5281/zenodo.21423111}), and the manuscript is deposited on arXiv (\texttt{math.NT}, cross-list \texttt{math.DS}, arXiv ID pending).
